...
| 2 | **GEMM multi-riga con riuso vero** — la leva misurata a 43,1x sul kernel | **(a0) DUE CELLE PRIMA DI COSTRUIRE** (fase 0 residua, docket item 12c + item 11, autorizzato dal PI): `splitk-coldw` — la vincitrice a pesi freddi, che discrimina se il suo vantaggio è traffico o occupancy, e che la riga 1 non ha girato — e `splitk-idot`, la stessa forma con `packed_4x8_integer_dot_product` al posto del dequant in virgola mobile. Se `idot` vince, è LEI la forma che va in produzione; (a) il prefill del 4B smette di usare `gemvQuantWgsl` con `batch: true` (= M GEMV replicate su `wid.z`) e adotta la variante vincente di riga 1; (b) **RIUSO, misurato non dedotto**: byte di peso emessi per token prefillato scende di **≥ 5,5x a M ≥ 16 sull'INVENTARIO PER-LAYER INTERO**, nello stesso JSON PRIMA/DOPO. **Il ≥ 8x precedente era irraggiungibile a qualunque M praticabile** (tetto 8,67x, servirebbe M ≥ 92): la forma nuova è q4_0-only e l'11,54% dei byte del 4B — 24 `ssm_out` Q5_K + 4 `ffn_down` Q4_1 — resta sul percorso vecchio. Misurato 5,86x. Ruling PI 2026-08-13, docket item 19. **Il residuo è scope del goal K-quant, e va NOMINATO nel consuntivo, non lasciato implicito**; (c) `prefill.tokS` sul prompt-idx 0 PRIMA e DOPO nello stesso JSON; (d) **copertura della convenzione dichiarata**: quanti dei siti di prefill la adottano su quanti esistono, con worklist ed eccezioni motivate — una convenzione applicata al 30% è il difetto, non il progresso; (e) conformità: il gate `q35-prefillchunk-4b` resta **bit per bit** contro `step()` sequenziale, oppure la tolleranza si dichiara PRIMA con la ragione numerica; (f) argmax del golden 4B IDENTICO al pin | `sdd-conductor` (build multi-task: kernel + assemblatore + gate + fixture) → poi `second-opinion` sul kernel numerico | none | `src/engine/kernels/**`, `src/engine/q35gpumodel.ts`, `src/engine/prefillplan.ts`, `tests/**`, `results/engine/**` | 3 it | **in corso (it.14)**: cablaggio VERIFICATO e mergiato (`7bc6b55`) — il ktest dopo il riavvio fa 100 PASS/0 FAIL sullo stesso ramo che ieri moriva, quindi la causa era l'infrastruttura. Misurato sul prompt-idx 0: prefill **34,36 → 111,16 tok/s = 3,24×** sul braccio chunked M=16, TTFT a caldo **87.618 → 56.984 ms = 1,54×**, decode 47,17 (−1,30%, in banda). (b) riuso **5,8593×** sull'inventario per-layer INTERO ≥ 5,5× del ruling ✓ · (c) prima/dopo ✓ (baseline it.1 + artefatto it.14) · (e) **NON VERIFICATA — la spunta che avevo messo in it.16 era DEDOTTA dalla prosa, non eseguita**: `q35-prefillchunk-4b` non è un banco del ktest, vive in `q35conf.worker.ts:386` e si attiva con `--prefill-m` SENZA `--prompt-idx`. Girato in it.17 per la prima volta dall'it.14: **`bitIdentical: false`**, e l'esperimento di attribuzione dice che a romperla è stata la **via intera di it.15** (attenzione legacy + idot: maxAbs 1,80), non l'attenzione (che porta a 2,38). Docket item 22 · (f) argmax golden 4B ✓ sul path di decode (`q35-model-4b-argmax` 6/6), **non** sul path prefill-chunk. **(a0) CHIUSA in it.15** (`b8ad31f`): il call-site ri-derivava a mano la condizione e finiva sempre sulla f32 mentre `prefillgemmplan.ts` — completo, coi suoi test — **non era importato da nessuno**. Ora `gemvB` chiede la rotta a `planPrefillGemm` e instrada `prefillQuantXQ8Wgsl` + `prefillGemmQ4SplitKIdotWgsl` dove la language feature c'è. Misurato: prefill **123,26 tok/s**, TTFT a caldo **51.392 ms** ⇒ **1,70× sulla baseline**; l'end-to-end della via intera rende **1,119×**, non l'1,745× del micro-bench, perché i moltiplicatori sono solo una parte del prefill. Gate ri-verificati in it.16 da sessione indipendente: tsc pulito, vitest 645\|10, ktest 100 PASS/0 FAIL. **(d) CHIUSA in it.19**: copertura per CALL-SITE, non per byte, pinnata nel test `[6f]` — la rotta si chiede al piano da **un solo posto** (`gemvB`), nessun sito ri-deriva a mano la condizione della via veloce, e la worklist delle eccezioni è esatta (5 emissioni su 4 siti logici: `gemvF32Wgsl` ×2, i tre K-quant sullo stesso ternario). **Correzione al censimento di it.17**: i kind che restano legacy sono **tre**, non due — avevo dimenticato i 24 `ssm` Q8_0, che però **passano dall'imbuto** e chiedono la rotta: è il piano a rispondere «legacy» perché la via veloce è q4_0-only. Coperto dalla convenzione ≠ instradato sulla via veloce, e il test tiene ferma la distinzione. **RIGA 2 COMPLETA** |
| 3 | **Attenzione a chunk del prefill** — gli stessi tre difetti già chiusi sul decode | (a) `attnDecodeWgsl` con `batch: true` smette di instradare al legacy (`wgsl.ts:530`): softmax in streaming (niente `scores: array<f32, ctxMax>`), KV letta una volta per gruppo GQA, letture vec4; (b) ms per chunk a ctx 388 **e** a ctx ≥ 6000 PRIMA e DOPO nello stesso JSON; (c) il golden `tests/fixtures/attn-decode-legacy.golden.json` aggiornato DELIBERATAMENTE, con la ragione nel journal — non per sbaglio; (d) ktest del prefill PASS contro cpuref con la tolleranza dichiarata PRIMA (cambia l'ordine delle somme: **non bit-identico per costruzione**, come la riga 1 del goal precedente); (e) argmax del golden 4B IDENTICO al pin | `sdd-conductor` → `second-opinion` sul kernel numerico | none | `src/engine/kernels/**`, `src/engine/q35gpumodel.ts`, `tests/**`, `results/engine/**` | 2 it | **quasi done (it.17-18)** via `sdd-conductor` `wf_991df002-d1d`, che **ha completato** — la conclusione «il veicolo non regge qui» era una diagnosi sbagliata di una causa infrastrutturale (docket item 23). (a) ✓ il ramo batch non instrada più al legacy: softmax in streaming a tile di 64, letture vec4, **niente fusione GQA** (docket item 21, esclusa coi numeri: 2,0879 contro 1,8207 ms) · (b) ✓ AC2, la sonda porta i due bracci `legacy` e `prod` nello stesso JSON, con un test senza GPU che verifica che siano davvero due kernel diversi · (c) ✓ golden aggiornato deliberatamente · (d) ✓ tolleranza dichiarata PRIMA (rel 1e-4 / abs 1e-5, ragione in loco) e ktest verde · (e) ✓ argmax del golden 4B. **MISURA: prefill 123,26 → 196,41 tok/s (1,59×), TTFT a caldo 51.392 → 32.265 ms ⇒ 2,72× sulla baseline**, artefatto `results/engine/ttft-riga3-4b-attnstream-prompt0-2026-08-14.json`. Gate rifatti in it.18 da sessione indipendente: tsc pulito, vitest 671\|10, ktest **101** PASS/0 FAIL, decode 47,89 (≥ 45,5). **RESIDUO**: la tolleranza vive come due costanti LOCALI invece che in `attnchunktol.ts` — T2 BLOCKED per ownership sovrapposta con T1 sullo stesso blocco (docket item 23); e la clausola (d) della riga 2 (censimento fatto in it.17, da scrivere) |
| 4 | **Portabilità (e2)** — con lo scoping corretto da (C7-3) | (e2b, percorso di prodotto) `engineNeeds` CONTA il ramo `batch` dell'attenzione: dopo la riga 3 il suo fabbisogno è costante in ctxMax e dichiarato, e `tests/gpulimits.test.ts` lo asserisce come garanzia invece che come debito. (e2a, percorso di conformità 0.5B) `rmsPairGemmSiluChunkFast` (`4·K·mMax + 256·mMax + 16·mMax`, K=896: 30.848 B a mMax=8, **61.696 a mMax=16**) — o scende sotto 16.384 B, o la clausola si dichiara **debito del path 0.5B** con la ragione, e il done-when del contratto va a docket per un ruling. **NOTA: alzare mMax PEGGIORA (e2a) — le due leve tirano in direzioni opposte, ed è la riga 1 a dover trovare una forma il cui shared non scali con M** | diretto | none | `src/engine/gpulimits.ts`, `tests/gpulimits.test.ts`, `src/engine/kernels/**` | 1 it | **done (it.22)**. **(e2b) CHIUSA da T1**: il ramo `batch` dell'attenzione è il quinto consumatore **dichiarato** in `engineNeeds`, con fabbisogno **costante in ctxMax** e uguale a quello del decode; `tests/gpulimits.test.ts` lo asserisce come **garanzia** — il `describe` «il ramo batch dell'attenzione e' contato e non alza il requisito» è letteralmente il vecchio sensore del debito, rovesciato. Contarlo non alza il requisito di un byte. **(e2a) DICHIARATA DEBITO** del path di conformità 0.5B, con l'aritmetica (`4·K·mMax`, K=896 ⇒ sotto 16.384 servirebbe mMax ≤ 4, e alzare mMax è la leva opposta) e la via d'uscita (portare al path 0.5B la forma multi-riga del 4B, il cui storage non scala con M). **(C7-3) ri-verificata sul codice di oggi, non ereditata**: `q35gpumodel.ts` non importa né il kernel né `prefillplan.ts`. Il debito è ora **falsificabile**: il test `(e2a)` fallisce se qualcuno cabla il kernel fuso nel 4B. Il testo del DONE WHEN del contratto è a docket item 25 per il PI. Gate: tsc pulito, vitest **681\|10**, `gpulimits.test.ts` 32/32; ktest non rieseguito perché la riga non tocca kernel né worker (soli commenti + un test) |
