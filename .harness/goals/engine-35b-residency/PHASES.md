...
sul pass stesso — dove `readbackWait` era il **94%** del token pulito. Con un
token pulito a ~21 ms le righe 2 e 3 bastano. **I due mondi si distinguono con
UNA misura**, ed è dentro la riga 1: riprodurre la forma di it.35 sull'albero di
oggi (39 token, arena 12 GiB, passata calda a zero miss) e leggere `tokenMs`.
Finché quel numero non esiste, la riga 2 non parte — non per prudenza, ma perché
sceglierebbe il meccanismo sbagliato nella metà dei casi.
