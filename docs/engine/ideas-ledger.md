...
| Floor dispatch ≤100/token | split B1/B2 (PI 2026-07-29) | B2 | ~~ri-inquadrato su sync/encode (73% fuori GPU)~~ CORRETTO 2026-07-30 (attribuzione fase 1 B2): il 73% era un artefatto da contesti diversi; quota fuori-GPU reale al ctx bench ~20%, GPU busy scala con kvLen (attention decode 6.46 ms/token a ctx ~570 vs 2.43 a ctx ~64) ⇒ la leva primaria è il kernel attention (split sul contesto), poi sync (~1.6 fissa) e floor GEMV/dispatch — ruling re-scope pendente (docket goal B2 item 2) | decode-attrib-4090-*2026-07-30*.json; tsq-diag §Conseguenze (correzione) |
