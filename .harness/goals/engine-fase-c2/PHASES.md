...
| 5 | MoE: router (sigmoid+bias+top-4+norm, replica C1) + GEMV dequant-fusa per-expert + shared expert + residenza minima (staging RAM, cache VRAM LRU ~2.573 slot, telemetria hit/upload) | conformance routing vs traccia C1 ≥ soglia spec su corpus C1; unit verdi su router/cache; contatore upload RAM→VRAM presente nel report telemetria | none | src/engine/ (moe/residenza nuovi file), tests/ | in progress (slice 1 done it.6) |
