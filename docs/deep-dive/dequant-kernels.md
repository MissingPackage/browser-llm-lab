...
|---|---|---|---|---|
| 4090 laptop | ~576 GB/s `[VERIFY: datasheet]` | ~2000 tok/s | 101–116 tok/s (`results/4090-linux-2026-07-26T19-54-55-278Z.json`) | **~5%** |
| S22 Ultra | ~51 GB/s LPDDR5 `[VERIFY: datasheet]` | ~180 tok/s | 6.99 tok/s (`results/s22-ultra-2026-07-27T00-34-09-931Z.json`) | **~4%** |
...
2. **Ma in assoluto entrambi i device stanno a ~5% del tetto di banda pesi.** Il "decode
   ...
   nascosta nei GEMV. `[VERIFY: attribuzione non misurabile senza timestamp-query — è
   il compito del micro-bench di fase 6, che misurerà matmul isolati a taglie crescenti
   proprio per trovare dove il roofline si piega]`
...
e nel micro-bench. `[VERIFY: nessun run committato su modello >0.5B per confermare]`
...
| Block layout coalesced-access stile `mul_mat_q` di llama.cpp | llama.cpp, kernel quantizzati con layout a blocchi ottimizzato per warp/coalescing `[VERIFY: non letto il sorgente llama.cpp in questa sessione]` | Media-alta — tocca il layout pesi generato da TVM | Medio | engine-notes |
...
| Dequantizzare una volta e cachare i pesi f16/f32 in un buffer persistente tra token | Pattern da sistemi di offload (FlexGen) per batching, non per batch=1 `[VERIFY: nessun prior art per il caso singolo token]` | Bassa da implementare | Alto — a batch=1 quadruplica i byte da leggere/scrivere in VRAM, aggravando esattamente il bottleneck #1 | **scartata** (aggrava, non risolve) |
