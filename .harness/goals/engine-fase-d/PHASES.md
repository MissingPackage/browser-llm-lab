...
| 3b | **Resolve MoE su GPU** (ruling PI 2026-08-10, docket item 8) | il decode del 35B non fa piu' un readback per layer: router + selezione + risoluzione expert→slot su GPU via slotTable, kernel d'arena al posto dei sotto-range calcolati da CPU, e miss rilevato su GPU con repair+replay dalla CPU. DONE WHEN: (a) submit/token e readback/token MISURATI prima e dopo nello stesso JSON, con il caso a residenza piena a **1 submit/token**; (b) argmax IDENTICO al path attuale su un campione del golden 35B (gate secco); (c) ktest MoE q35 PASS invariati e GLM bit-identico; (d) il conteggio dei miss e il routing esatto restano quelli del path CPU sullo stesso campione | none | src/engine/**, tests/**, results/engine/** | **in corso: fette 1 (kernel d'arena, it.11), 2 (router+resolve WGSL, it.13), 3a (cablaggio arena, it.14), 3b (router in ombra sui layer veri, it.15), 3c (submit unico, it.17: 81→1 submit/token, 41→1 readback, argmax 39/39 e routing identici, −71,99 ms/token a parità) FATTE; resta il REPAIR+REPLAY, che la riga chiede esplicitamente — e la sua precondizione è nuova: 30 layer su 40 sono ricorrenti (deltanet in-place), quindi il replay di GLM non è portabile senza checkpoint di stato (it.17)** |
