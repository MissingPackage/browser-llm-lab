...
| 3 | Decode multi-step senza readback (**densi done it.10; MoE → docket item 8**) | q35 (densi + MoE) usa il pattern `decodeBatch` (argmax on-GPU + embed gather, K step/submit): micro-bench ms/token prima/dopo (atteso ≥ −5.1 ms dal q1); argmax IDENTICO al path a readback su un campione (gate di correttezza secco) | none | src/engine/**, tests/**, results/engine/** | **densi done, MoE aperto** |
