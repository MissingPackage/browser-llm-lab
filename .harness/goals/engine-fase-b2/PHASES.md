...
| 4 | **Decode loop multi-step K + token-identity** — K forward/submit, feedback token on-GPU (o fallback da spec), argmax in catena, readback 1/K; percorso per-token tenuto vivo come oracolo interno fino a fase 6 | unit CPU-side sul piano del loop verdi; report JSON token-identity: K>1 vs per-token, greedy, ≥256 token IDENTICI, exit 0; conformance exit 0 attraverso il nuovo loop | none | `src/engine/**`, `tests/`, `scripts/` | **ready** (fase 3 done 2026-07-30) |
