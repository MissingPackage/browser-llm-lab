...
| 5 | **Telemetria liv.2 + profiler nel nuovo loop** — tsq ring compatibile con K/submit (mapAsync SOLO post-submit), finestra decode del profiler aggiornata | run liv.2 con gpuMs reale non-null (JSON); conformance exit 0 con telemetryGpu:true; profiler JSON con submit/token e dispatch/token; target dispatch di spec rispettato O archiviazione motivata nel ledger §I | none | `src/engine/**` (percorso telemetria), `scripts/`, `results/engine/`, `docs/engine/ideas-ledger.md` (§I) | **ready** (fase 4 done 2026-07-30) |
