...
| 3 | **Forward multi-token M≤8 + parità** — kernel/piano per chunk di prefill (GEMM small-batch o loop M sul piano fuso, decisione in spec), tap preservati | `npm test` verde con unit CPU-side sul piano di chunking; `scripts/conformance-engine.mjs` exit 0 (gate doppio fase A) col percorso prefill M>1 attivo | none | `src/engine/**`, `tests/`, `scripts/` | ready (ruling spec 2026-07-30) |
