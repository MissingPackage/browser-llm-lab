...
| 3 | Decode multi-step senza readback (**densi: -3,33 ms/token misurato, SOTTO il >= -5,1 richiesto → docket item 9; MoE → fase 3b**) | q35 (densi + MoE) usa il pattern `decodeBatch` (argmax on-GPU + embed gather, K step/submit): micro-bench ms/token prima/dopo (atteso ≥ −5.1 ms dal q1); argmax IDENTICO al path a readback su un campione (gate di correttezza secco) | none | src/engine/**, tests/**, results/engine/** | **densi misurati (docket 9), MoE → 3b** |
| 3b | **Resolve MoE su GPU** (ruling PI 2026-08-10, docket item 8) | il decode del 35B non fa piu' un readback per layer: router + selezione + risoluzione expert→slot su GPU via slotTable, kernel d'arena al posto dei sotto-range calcolati da CPU, e miss rilevato su GPU con repair+replay dalla CPU. DONE WHEN: (a) submit/token e readback/token MISURATI prima e dopo nello stesso JSON, con il caso a residenza piena a **1 submit/token**; (b) argmax IDENTICO al path attuale su un campione del golden 35B (gate secco); (c) ktest MoE q35 PASS invariati e GLM bit-identico; (d) il conteggio dei miss e il routing esatto restano quelli del path CPU sullo stesso campione | none | src/engine/**, tests/**, results/engine/** | ready |
