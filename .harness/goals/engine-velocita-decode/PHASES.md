...
| 2b | **IL RAGGRUPPAMENTO DELLE RICHIESTE DI I/O — leva globale, chiesta dal PI.** Misurato in it.2 e non assunto: **2,1×** fra 6,90 ms/fetch (24 richieste concorrenti, `prepLayer`) e 3,27 ms (qualche centinaio, il repair). Stessi byte, stesso server, stesso `readExpert`: cambia solo il raggruppamento. Va messo nel path di I/O **condiviso** (`range()`/`readRange`, `chat.worker.ts:62`, `q35conf.worker.ts:186`), non nei due call site del 35B — altrimenti è la «piccolezza specifica» che il ruling vieta. Candidati, da misurare non da assumere: coalescing delle Range adiacenti in una richiesta sola (gate+up+down di un expert sono tre tensori diversi, quindi tre offset: il coalescing vero è **fra expert dello stesso tensore**), finestra di concorrenza esplicita, sorgente non-HTTP (OPFS `FileSystemSyncAccessHandle`). | tempo per fetch **< 1,5 ms** (oggi 3,27 nel repair, 6,90 in prep) sul contatore della riga 1; **l'effetto misurato ANCHE sul LOAD di 4B, 9B e GLM**, che passano dallo stesso `range()` — se lì non paga si dichiara col numero invece di tacere; nessuna regressione dei gate di correttezza. | none | `src/engine/chat/chat.worker.ts`, `src/engine/q35conf/**`, `src/engine/q35expertstore.ts`, `src/engine/residency.ts`, `src/engine/ggufrange.ts` | **misura COMPLETA (it.30-36), done-when in attesa di ruling (docket item 8)** — cinque esperimenti controllati sullo stesso lettore e lo stesso file: il lettore e' passato da 5 copie a 1 (e una aveva perso il controllo di lunghezza, che ha poi parato due letture vuote vere); i byte per chiamata sono uguali nei due regimi (1,7826-1,7847 MB), quindi il 2,1x non era un artefatto di taglie; la curva banda/richieste-in-volo ha il ginocchio a 2-4 e il tetto a ~700 MB/s, **quindi il done-when a 1,5 ms/fetch non e' raggiungibile su HTTP** (servono 1.189 MB/s; OPFS ne da' 1.372); continuo contro raffiche 0,98x a 24, **cioe' raggruppare NON paga**; parallelismo fino a 136,5 e localita' irrilevante, quindi niente tetto di connessioni. **La domanda del PI («se puo' dare un boost globale») ha risposta: NO su questo trasporto.** Il residuo — 3,2x di latenza per lettura dentro il motore a parallelismo confrontabile — e' materia della riga 3. | 1-2 it, **7 consumate** (di cui 5 di misura) |
