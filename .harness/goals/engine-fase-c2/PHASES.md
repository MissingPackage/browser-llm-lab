...
| 6 | E2E: forward GLM completo nel piano statico, conformance logits full-model, bench con i GATE (decode ≥13.4, prefill ≥56.6 tok/s) + non-regressione Qwen (conformance A + bench first-light ≥ ultimi valori committati) | run conformance exit 0 + JSON in results/engine/ entro soglie; bench JSON con entrambi i gate tok/s PASS; bench+conformance Qwen ri-eseguiti ≥ valori 2026-07-30 | none | src/engine/, results/engine/ | in progress (slice 1 conformance DONE it.10; slice 2 bench next) |
