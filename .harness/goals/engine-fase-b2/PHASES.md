...
| 6 | **Bench + non-regressione + chiusura** | bench JSON (schemaVersion 3, baseline same-day): decodeToksPerSec.mean ≥ soglia di spec E prefillMs mean ≤ 810 E overhead telemetria spenta ≤2%, con quota fuori-GPU/gpuBusy riportati; kv-rollback + prefix-cache re-run exit 0 (JSON); checklist DONE WHEN 7/7 nel journal; HANDOFF aggiornato; merge+push a goal chiuso (may-do, ruling permanente) DOPO verifier gate PASS | none | `results/engine/`, `HANDOFF.md`, docket, journal | **ready** (fase 5 done 2026-07-30) |
