...
| Picco di load = stato finale (upload a chunk con backpressure; OPFS SyncAccessHandle come SSD) | — | **misurata (2026-07-29, 4090/NVMe)**: write 2.2 GB/s; read via API 7.5-11.7 GB/s a page-cache calda ⇒ l'overhead API non è il bottleneck, il freddo è disco-bound. **Bound freddo lato OS misurato (2026-07-31, fadvise)**: random expert-size 1.63 GB/s (p50 3.74 ms/expert), seq 3.22 GB/s (`tools/cold-read-bench.py`). **Freddo BROWSER misurato (2026-08-08, goal c3c fase 1, `tools/opfs-cold/`)**: random expert-size **1.79-1.94 GB/s (p50 2.9-3.1 ms/expert, p95 3.7-4.5)**, seq 1 MiB 3.43-3.73 GB/s, streaming sequenziale expert-size 3.51-3.66 GB/s (p50 1.3-1.4 ms/expert — il regime instant-on) ⇒ **parità col bound OS (ratio 1.07-1.19)**: il freddo browser NON è "≤" per tassa API, è lo stesso numero; freddezza provata nel protocollo (fincore 0 B post-drop, delta warm 8× sugli stessi offset). 2 run: `results/opfs-bench/opfs-cold-4090-linux-2026-08-08*.json` |
...
| **Soglia di high-variance separata per il TTFT** nel benchmark harness: il flag guarda solo `decodeToksPerSec`, e su un run S22 reale il TTFT oscillava del 104% mentre il flag taceva | fase-2-deep-dive it.9 (2026-07-27), rimandato a un "goal consolidamento" **mai aperto** | primo goal che rimette mano al benchmark harness | si riapre lavoro su `src/benchServer.ts` o si rifà uno sweep multi-device | fase-1b-matrice docket #12; fase-2-deep-dive docket #9; `src/benchServer.ts:9,101,104` |
| **`quality.ts` costruito, testato e mai collegato** a `benchServer.ts` (`qualityScore` è dichiarato in `src/schema.ts:56` e non popolato da nessuno) | fase-1b-matrice (2026-07-27), lasciato aperto alla chiusura | idem | idem | fase-1b-matrice docket #10 |
| **Bug upstream wllama** (ngxson/wllama#263/#264) accettato con "controllo ogni 3 giorni" **mai meccanizzato** (nessuna automazione nel repo) | fase-1b-matrice (2026-07-27) | quando il benchmark harness torna in scope | rilascio upstream che chiude le issue, oppure decisione di droppare wllama | fase-1b-matrice docket #8 |
| **Prefill BATCHED q35** (leva n.1 del WP gap q1: 183× = assenza, non gap; pattern GLM chunked in casa) | q1 (2026-08-10, misurato non implementato) | fase D sul modello target | primo goal di performance sulla famiglia q35 | docket q1 item 10; study/2026-08-10-q35-gap-decomposition.md §3-4 |
| **Decode multi-step no-readback su q35** (−5.1 ms/token MISURATI = 12% del decode 4B; pattern GLM decodeBatch B2) | q1 (misurato) | fase D | insieme al prefill batched | gap-decomposition §2; docket q1 item 10 |
| **Int8 packed: dot4I8Packed ⇄ subgroup-matrix CONVERGONO** (probe: feature esposta sul nostro harness ma INT8-ONLY 16×16×32; la misura richiede attivazioni int8 packed = stessa infra) | q1 fase 6 (probe committato) | post-prefill-batched | quando il matmul batched esiste e il collo torna ALU | docket q1 item 11; results/engine/webgpu-subgroup-matrix-probe-2026-08-10.json |
| **Rifit meccanica C3c piena sul MoE q35** (tier.h/AUTOPIN/prefetch/slab pre-repacked; oggi LRU pura on-miss con repack JS: costo miss 17.64 ms fit) | q1 (paging minimale deliberato, GLM residency INTATTO) | fase D sul modello target | il goal di performance q35 la porta; recall lookahead 82.67%@8 (< GLM 91.92%, spiegato) ridimensiona il prefetch | docket q1 item 10-12; q35-bandmodel-fit; q35-looka JSON |
