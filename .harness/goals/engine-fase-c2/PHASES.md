...
| 6 | E2E: forward GLM completo nel piano statico, conformance logits full-model, bench con i GATE (decode ≥13.4, prefill ≥56.6 tok/s) + non-regressione Qwen (conformance A + bench first-light ≥ ultimi valori committati) | run conformance exit 0 + JSON in results/engine/ entro soglie; bench JSON con entrambi i gate tok/s PASS; bench+conformance Qwen ri-eseguiti ≥ valori 2026-07-30 | none | src/engine/, results/engine/ | misurata, gate NON passati (it.10 conformance PASS; it.11 bench decode 3.30/13.43 e prefill 4.41/56.58 FAIL con attribuzione; Qwen conformance PASS, K=8+prefill sotto soglia attribuiti all'host) → docket item 6 e 7, ruling PI |
