...
| 6 | **Bench + non-regressione + chiusura** — prefill vs baseline fase A, decode invariato, contatori invariati | bench JSON in `results/engine/`: prefillMs mean ≤ soglia di spec (default 1/2 di ~2.44s) E decodeToksPerSec.mean ≥ 120; JSON profiler: createBindGroup=0, submit/token=1, dispatch/token ≤130; checklist DONE WHEN 8/8 nel journal; HANDOFF aggiornato; merge+push a goal chiuso (may-do, ruling permanente) DOPO verifier gate PASS | none | `results/engine/`, `HANDOFF.md`, docket, journal | **done** (2026-07-30, it.6: prefill 3.44x, decode 122.4, profiler 0/1/123.6, checklist 8/8 — GOAL CHIUSO) |
